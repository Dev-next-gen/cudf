...
if (( $# == 0 )); then
  set -- cpp/cmake/thirdparty/get_*.cmake java/src/main/native/cmake/thirdparty/get_*.cmake
fi

for cmake_file in "$@"; do
  if sed -E 's/[[:space:]]*#.*$//' "${cmake_file}" |
    grep -nE '(^|[^[:alnum:]_])GIT_(REPOSITORY|TAG|SHALLOW)([^[:alnum:]_]|$)'; then
